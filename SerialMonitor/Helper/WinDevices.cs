...
{
    private readonly Regex _regex = new(@"\((COM\d+)\)", RegexOptions.IgnoreCase | RegexOptions.Compiled);
    ...
    {
        var devices = new ObservableCollection<ComPortInfo>();
        ...
        try
        {
            var deviceInfoData = new Interop.SP_DEVINFO_DATA();
            ...
            {
                var friendlyName = GetDeviceProperty(_deviceInfoSet, ref deviceInfoData, Interop.SPDRP_FRIENDLYNAME);
